...
export class xgn extends plugin {
  ...
  async gangqin (e) {
    ...
    const ffmpeg = spawn('ffmpeg', msg, {
      cwd: './plugins/Fl-Plugin/resources/tanqin/' + path
    });
    ...
    ffmpeg.on('close', async (code) => {
      ...
      kg = 1
      if (kg == 1) {
        // let msg2 =  segment.record('./plugins/Fl-Plugin/resources/tanqin/output2.mp3')
        await sleep(2000)
        try {
...
async function SendMusicShare (e, data, to_uin = null) {
  ...
  typeof (data.url) == 'function' ? musicUrl = await data.url('/SmallK111407/Fl-Plugin/edit/master/apps/data.data') : musicUrl = data.url;
